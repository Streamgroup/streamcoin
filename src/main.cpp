...
unsigned int nStakeMaxAge       = 10 * 12 * 30 * 24 * 60 * 60;	// 
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // streamcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // streamcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// streamcoin: find block wanted by given orphan block
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, uint256 prevHash)
{
	...
	if (nHeight <= 0)
        ...
        nSubsidy = 2000000 * COIN;	// Hold for develop and promotion
        else if (nHeight <= 10000)
        nSubsidy = 1 * COIN;      // SUPER
        else if (nHeight <= 20000)
        nSubsidy = 2 * COIN;      // SUPER
        else if (nHeight <= 30000)
        nSubsidy = 3 * COIN;      // SUPER
        else if (nHeight <= 40000)
        nSubsidy = 4 * COIN;      // SUPER
        else if (nHeight <= 50000)
        nSubsidy = 5 * COIN;      // SUPER
        else if (nHeight <= 60000)
        nSubsidy = 6 * COIN;      // SUPER
        else if (nHeight <= 70000)
        nSubsidy = 7 * COIN;      // SUPER
        else if (nHeight <= 80000)
        nSubsidy = 8 * COIN;      // SUPER
        else if (nHeight <= 90000)
        nSubsidy = 9 * COIN;      // SUPER
...
// streamcoin: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // streamcoin: target change every block
    // streamcoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // streamcoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // streamcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // streamcoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // streamcoin: track money supply and mint amount info
...
// streamcoin: total coin age spent in transaction, in the unit of coin-days.
...
// streamcoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // streamcoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // streamcoin: compute stake entropy bit for stake modifier
    ...
    // streamcoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // streamcoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // streamcoin: check proof-of-stake
    ...
    // streamcoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // streamcoin: getblocks may not obtain the ancestor block rejected
    ...
    // streamcoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "StreamCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "STREAMCOIN started 30.11.2016";
        CTransaction txNew;
        txNew.nTime = 1480519927;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1480519927;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 2252162 : 2252162;
...
//        assert(block.hashMerkleRoot == uint256("0xeb42ae0324ea968b5cdcd959104fc3b1cd93c45e25ba5be25b5e55deaf21254c"));
        assert(block.hashMerkleRoot == uint256("0x9a8184e753b1415fb6fedadce26513dffd5f9ddaab8a9d60a0ba6438ba796682"));
        ...
        // streamcoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // streamcoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // streamcoin: tell downloading node about the latest block if it's
